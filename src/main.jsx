...
const roastingLevels = [
  ...
  },
  {
    ...
    description: "Sarcastic, ruthless, technically accurate",
  },
];
...
function saveHtmlReport(result, level, files) {
  const roast = result.roast || {};
  const findings = files
    .map(
      (file) =>
        `<article class="finding"><div><code>${escapeHtml(
          file.path
        )}</code><span class="${escapeHtml(file.severity)}">${escapeHtml(
          file.severity
        )}</span></div><p>${escapeHtml(file.comment)}</p></article>`
    )
    .join("");
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Repo Roaster report — ${escapeHtml(
    result.repo
  )}</title><style>body{margin:0;padding:40px;background:#101412;color:#dfe8df;font:16px Arial,sans-serif;line-height:1.5}.report{max-width:800px;margin:auto}.eyebrow{color:#a3df56;font:12px monospace;letter-spacing:.08em}h1{font-size:28px;margin:12px 0}h2{font-size:20px;margin-top:32px;color:#a3df56}.meta{color:#9dab9e;font-size:13px}.summary,.finding,.verdict{border:1px solid #3f5044;background:#141b17;padding:18px;margin-top:14px;border-radius:4px}.summary{font-size:20px}.finding code{color:#d9e6d9}.finding span{float:right;text-transform:uppercase;font:11px monospace;padding:3px 6px;border:1px solid}.finding span.high{color:#ff8a72}.finding span.medium{color:#e1b054}.finding span.low{color:#a3df56}.finding p{color:#b5c4b7;margin-bottom:0}.verdict strong{display:block;font-size:18px;margin-top:6px}@media(max-width:600px){body{padding:22px}}</style></head><body><main class="report"><div class="eyebrow">REPO ROASTER / ${escapeHtml(
    level?.name || "ROAST"
  )}</div><h1>${escapeHtml(result.repo)}</h1><div class="meta">${escapeHtml(
    result.filesReviewed
  )} source files inspected · Technical debt: ${escapeHtml(
    roast.debtLevel
  )}/10</div><section class="summary"><strong>${escapeHtml(
    roast.summary
  )}</strong></section><h2>Findings</h2>${
    findings || "<p>No findings reported.</p>"
  }<section class="verdict"><span>FINAL VERDICT</span><strong>${escapeHtml(
    roast.verdict
  )}</strong></section></main></body></html>`;
  const blob = new Blob([html], { type: "text/html" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `repo-roaster-${
    result.repo.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "") || "report"
  }.html`;
  link.click();
    ...
    <div className="report">
      <div className="report-top">
        <div>
          ...
          <p>
            {result.filesReviewed} source files inspected ·{" "}
            {level?.name || "Roast"} · verdict generated by AI
          </p>
        </div>
        <div
      ...
      <button
        ...
        onClick={() => saveHtmlReport(result, level, files)}
      >
        SAVE HTML REPORT <span>↓</span>
      </button>
    </div>
  );
}
